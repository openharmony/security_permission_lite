...
public:
    ...
    void AddPermissionsRecord(const std::string &permissionName, const std::string &deviceId, int32_t uid,
        int32_t sucCount, int32_t failCount);

    int32_t GetPermissionRecordsCompress(const std::string &queryGzipStr, std::string &resultStr);

    int32_t GetPermissionRecordsAsync(const std::string &queryGzipStr, const sptr<OnPermissionUsedRecord> &callback);

    void DeletePermissionUsedRecords(int32_t uid);
...
private:
     ...
     */
    bool AddVisitor(const std::string &deviceId, int32_t uid, int32_t &visitorId);
     ...
     * @return true if succeeded; false if failed.
     */
    bool GetRecordFromDB(int32_t allFlag, const std::vector<GenericValues> &recordValues,
     ...
     * @return 0 if succeeded; -1 if failed.
     */
    int32_t DeletePermissionRecords(int32_t days);
     ...
     * @return true if succeeded; false if failed.
     */
    bool GetPermissionVisitor(const std::string &deviceId, int32_t uid, PermissionVisitor &permissionVisitor);
