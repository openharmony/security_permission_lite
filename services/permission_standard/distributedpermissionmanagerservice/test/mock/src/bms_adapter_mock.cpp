...
namespace {
static constexpr OHOS::HiviewDFX::HiLogLabel LABEL = {LOG_CORE, SECURITY_DOMAIN_PERMISSION, "BmsAdapterMock"};
}
sptr<AppExecFwk::IBundleMgr> BmsAdapter::GetBundleManager()
{
    PERMISSION_LOG_DEBUG(LABEL, "BmsAdapter GetBundleManager called");
    if (iBundleManager_ == nullptr) {
        auto bundleObj = new OHOS::AppExecFwk::BundleMgrService();
