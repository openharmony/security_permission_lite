...
namespace {
static constexpr OHOS::HiviewDFX::HiLogLabel LABEL = {LOG_CORE, SECURITY_DOMAIN_PERMISSION, "PmsAdapterMock"};
}
sptr<Permission::IPermissionManager> PmsAdapter::GetPermissionManager()
{
    PERMISSION_LOG_DEBUG(LABEL, "PmsAdapter GetPermissionManager called");

    if (iPermissionManager_ == nullptr) {
        auto permissionObj = new PermissionManagerService();
