...
#include "get_permission_used_records_asynch_test.h"
...
void GetPermissionUsedRecordsAsynchTest::InitRecordData(int64_t timestamp)
{
    int opCode = 4;
    int count = 3;
    std::vector<GenericValues> visitor;
    ...
    genericRecordFore.Put(FIELD_ACCESS_COUNT, count);
    genericRecordFore.Put(FIELD_REJECT_COUNT, count);
    ...
    genericRecord.Put(FIELD_REJECT_COUNT, count);
...
void GetPermissionUsedRecordsAsynchTest::SetUpTestCase()
{
    GenericValues null;
...
void GetPermissionUsedRecordsAsynchTest::TearDownTestCase()
{
...
HWTEST_F(GetPermissionUsedRecordsAsynchTest, GetPermissionUsedRecordsAsynch_0100, Function | MediumTest | Level1)
{
    ...
    int flag = managerService.GetPermissionRecords(queryGzipStr, callback);
    int count = 3;
    sleep(count);
    EXPECT_EQ(flag, 0);
}
