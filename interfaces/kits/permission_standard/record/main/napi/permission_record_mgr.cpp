...
static void ConvertDetailRecord(
    napi_env env, napi_value objRecord, size_t idx, const BundlePermissionUsedRecord &bundleRecord)
{
    ...
    for (size_t ind = 0; ind < bundleRecord.permissionUsedRecords[idx].accessRecordBg.size(); ind++) {
        ...
        NAPI_CALL_RETURN_VOID(
            env, napi_create_int64(env, bundleRecord.permissionUsedRecords[idx].accessRecordBg[ind], &nAccessRecord));
    ...
    for (size_t ind = 0; ind < bundleRecord.permissionUsedRecords[idx].accessRecordFg.size(); ind++) {
        napi_value nAccessRecord;
        NAPI_CALL_RETURN_VOID(
            env, napi_create_int64(env, bundleRecord.permissionUsedRecords[idx].accessRecordFg[ind], &nAccessRecord));
    ...
    for (size_t ind = 0; ind < bundleRecord.permissionUsedRecords[idx].rejectRecordBg.size(); ind++) {
        ...
        NAPI_CALL_RETURN_VOID(
            env, napi_create_int64(env, bundleRecord.permissionUsedRecords[idx].rejectRecordBg[ind], &nRejectRecord));
    ...
    for (size_t ind = 0; ind < bundleRecord.permissionUsedRecords[idx].rejectRecordFg.size(); ind++) {
        napi_value nRejectRecord;
        NAPI_CALL_RETURN_VOID(
            env, napi_create_int64(env, bundleRecord.permissionUsedRecords[idx].rejectRecordFg[ind], &nRejectRecord));
...
static void ConvertRecord(napi_env env, napi_value objBundleRecord, const BundlePermissionUsedRecord &bundleRecord)
{
    napi_value nRecords;
    ...
    for (size_t idx = 0; idx < bundleRecord.permissionUsedRecords.size(); idx++) {
        ...
        ConvertDetailRecord(env, objRecord, idx, bundleRecord);
    ...
    napi_value objBundleRecord, const BundlePermissionUsedRecord &bundleRecord)
{
    ...
    bool ret = iBundleMgr->GetBundleInfo(bundleRecord.bundleName,
    ...
    ConvertRecord(env, objBundleRecord, bundleRecord);
}
...
static napi_value ParseInt64(napi_env env, int64_t &param, napi_value args)
{
    ...
    PERMISSION_LOG_INFO(LABEL, "param=%{public}." PRId64, value);
    param = value;
    // create result code
    napi_value result;
...
static napi_value ParseStringArray(napi_env env, std::vector<std::string> &hapFiles, napi_value args)
{
    ...
    // create result code
    napi_value result;
    napi_status status = napi_create_int32(env, 1, &result);
    NAPI_ASSERT(env, status == napi_ok, "napi_create_int32 error!");
    return result;
}
    ...
    const QueryPermissionUsedResult &queryResult)
{
    ...
    PERMISSION_LOG_INFO(LABEL, "ProcessQueryResult code=%{public}." PRId64, queryResult.beginTimeMillis);
    ...
    PERMISSION_LOG_INFO(LABEL, "ProcessQueryResult code=%{public}." PRId64, queryResult.endTimeMillis);
    ...
    for (const auto &bundle : queryResult.bundlePermissionUsedRecords) {
        ...
        for (const auto &record : bundle.permissionUsedRecords) {
            ...
            PERMISSION_LOG_INFO(LABEL, "lastAccessTime = %{public} " PRId64, record.lastAccessTime);
            PERMISSION_LOG_INFO(LABEL, "lastRejectTime = %{public} " PRId64, record.lastRejectTime);
        }
