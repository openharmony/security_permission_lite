...
sptr<IAbilityManager> STAbilityUtil::GetAbilityManagerService()
{
    ...
    if (!remoteObject) {
        HILOG_ERROR("fail to get ability manager proxy.");
        return nullptr;
    }

    HILOG_ERROR("get ability manager proxy success.");
...
sptr<IAppMgr> STAbilityUtil::GetAppMgrService()
{
    ...
    if (!remoteObject) {
        HILOG_ERROR("fail to get app manager proxy.");
        return nullptr;
    }

    HILOG_ERROR("get app manager proxy success.");
    return iface_cast<IAppMgr>(remoteObject);
}
